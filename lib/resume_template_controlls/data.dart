...
class AccessData {
  Future<TemplateData> getdata() async {
    TemplateData data = TemplateData(
      fullName: currentRessume?.personal[0] ?? '',
      street: currentRessume?.personal[1] ?? '',
      address: currentRessume?.personal[2] ?? '',
      country: currentRessume?.personal[3] ?? '',
      email: currentRessume?.personal[4] ?? '',
      phoneNumber: currentRessume?.personal[5] ?? '',
      // bio: currentRessume?.bio ?? '',
      // experience: currentRessume?.experience
      //     ?.map((exp) => ExperienceData(
      //           experienceTitle: exp['title'] ?? '',
      //           experiencePlace: exp['place'] ?? '',
      //           experiencePeriod: exp['period'] ?? '',
      //           experienceLocation: exp['location'] ?? '',
      //           experienceDescription: exp['description'] ?? '',
      //         ))
          // .toList() ?? [],
      educationDetails: currentRessume?.education
          .map((edu) => Education(edu['degree'] ?? '', edu['university'] ?? ''))
          .toList() ?? [],
      // languages: currentRessume?.languages
      //     ?.map((lang) => Language(lang['language'] ?? '', lang['level'] ?? 0))
      //     .toList() ?? [],
      // hobbies: currentRessume?.hobbies ?? [],
      // image: currentRessume?.profileimage ?? '',
      // backgroundImage: currentRessume?.backgroundimage ?? '',
    );
    return data;
  }
}
...
''';
// TemplateData data = TemplateData(
//     fullName: currentRessume?.personal[0] ?? 'Sanjaykrishnan Ku',
//     currentPosition: 'Flutter Developer',
//     street: 'kochi',
//     address: 'kizhakkedath parambu ',
//     country: 'india',
//     email: 'sanju.sanjaykrishnan@gmail.com',
//     phoneNumber: '+919656331062',
//     bio: bio,
//     experience: [
//       ExperienceData(
...
// void loadData() {
//   if (kDebugMode) {
//     print('${currentRessume?.education[0]}');
//   }
// }
